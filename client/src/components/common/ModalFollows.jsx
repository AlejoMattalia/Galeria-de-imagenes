...
export function ModalFollows({ open, setOpen, id, followingOrFollowers }) {
    ...
    try {
      const promises = users.map(async (user) => {
        ...
        const response = await axios.get(
          `https://galeria-imagenes-pi.vercel.app/api/user/show_image/${photo}`,
          {
            responseType: "blob",
          }
        );
  ...
  useEffect(() => {
    if (id) {
      if (followingOrFollowers === "following") {
        axios
          .get(`https://galeria-imagenes-pi.vercel.app/api/follow/following/${id}/${page}`)
          .then((res) => {
        ...
        axios
          .get(`https://galeria-imagenes-pi.vercel.app/api/follow/followers/${id}/${page}`)
          .then((res) => {
            if (res.data.follows.length === 0) {
              setHasMore(false);
            } else {
